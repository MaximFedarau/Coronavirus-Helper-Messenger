...
class ChatLogActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        send_button_chat_log.setOnClickListener {
            //Log.d(TAG,"Pressed button to send message")
            performSendMessage()
        }
    }
            ...
            override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                ...
                if (chatMessage != null) {
                    if (chatMessage.fromId == FirebaseAuth.getInstance().uid) {
                        ...
                        //Log.d("MyIcon","${currentUser.username}")
                        adapter.add(ChatFromItem(chatMessage.text,currentUser))
                    }
                    else {
                        val toUser_1 = intent.getParcelableExtra<User>(NewMessageActivity.USER_KEY)
                        //Log.d("testing","${toUser_1}")
                        adapter.add(ChatToItem(chatMessage.text,toUser_1!!))}
